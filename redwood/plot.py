...
os.environ.setdefault("MPLCONFIGDIR", os.path.join("/tmp", "redwood-matplotlib"))
...
# import the redwood rcParams
from redwood import rcparams as rc
from redwood.functions import GFFParse, print_images
from redwood.bamparse import BAMParse
...
def plot_reads(args, angleMap, widthDict, samFiledf, start_radius,
    ...
    while len(samFiledf) > 0:
        ...
        if not skip:
            ...
            samFiledf = samFiledf.reset_index()
            samFiledf = samFiledf.drop('index', axis=1)
        ...
        if i >= len(samFiledf):
            ...
            if args.interlace:
                ...
                samFiledf.drop('index', axis=1, inplace=True)
